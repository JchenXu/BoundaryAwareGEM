...
def local_feature(inputs, num_output_channels, bn, is_training, idx, bn_decay, weight_decay):
    """
    ...
    local_feature = tf.reduce_mean(local_feature, axis=2)
    #local_feature = conv2d_kcnet(inputs, 3*num_output_channels, [1, 1], padding='VALID', stride=[1, 1], bn=bn, is_training=is_training, scope='local%d'%(idx), bn_decay=bn_decay, weight_decay=weight_decay, activation_fn=None)
    return local_feature

def conv2d_kcnet(inputs,
           num_output_channels,
           kernel_size,
           scope,
           stride=[1, 1],
