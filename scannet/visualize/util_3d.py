...
def load_ids_boundary(filename):
    ids = open(filename).read().splitlines()
    ids_ = np.array([i.split(' ')[0] for i in ids], dtype=np.int64)
    boundary = np.array([i.split(' ')[1] for i in ids], dtype=np.int64)
    boundary_possi = np.array([i.split(' ')[2] for i in ids], dtype=np.float32)
    return ids_, boundary, boundary_possi
