...
def write_ply_label(points, labels, filename, num_classes=None, text=True):
    ...
    colors = np.array([pyplot.cm.hsv(i/float(num_classes)) for i in range(num_classes)])
...
def getColor(labels, num_classes):
    colors = np.array([pyplot.cm.hsv(i/float(num_classes)) for i in range(num_classes)])
    ...
    colors = np.array(new_colors, dtype = np.float32)

    return colors

def write_ply_label2(points, labels, filename, num_classes=None, text=True):
    """ Color (N,3) points with labels (N) within range 0 ~ num_classes-1 as OBJ file """
    import matplotlib.pyplot as pyplot
    labels = labels.astype(int)
    N = points.shape[0]

    colors = getColor(labels, num_classes)
    c0=colors[:,0]
    c1=colors[:,1]
    c2=colors[:,2]
    ...
    points = [(points[i,0], points[i,1], points[i,2], c0[i], c1[i], c2[i]) for i in range(points.shape[0])]
