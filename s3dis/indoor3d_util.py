...
DATA_PATH = os.path.join(ROOT_DIR, 'dataset', 'Stanford3dDataset_v1.2')
...
def sample_data(data, num_sample):
    ...
    if (N == num_sample):
        ...
        return np.concatenate([data, dup_data], 0), [i for i in range(N)]+list(sample)
...
def room2blocks(data, label, num_point, block_size=1.0, stride=1.0,
    ...
    if not random_sample:
        num_block_x = int(np.ceil((limit[0] - block_size + 0.5) / stride)) + 1
        num_block_x = max(num_block_x, 1)
        num_block_y = int(np.ceil((limit[1] - block_size + 0.5) / stride)) + 1
        num_block_y = max(num_block_y, 1)
        for i in range(num_block_x):
            for j in range(num_block_y):
                xbeg_list.append(i*stride - 0.25)
                ybeg_list.append(j*stride - 0.25)
    else:
    ...
    for idx in range(len(xbeg_list)): 
       ...
       block_data_sampled[:,0] -= (xbeg + block_size/2)
       block_data_sampled[:,1] -= (ybeg + block_size/2)
